...
function getCpuDataSheet(cpuId)
{
    ...
    if (cpuId == "0") {
        return (Utils.getCpuDataSheet(this.$module.T16));
    }
    else if (cpuId == "1") {
...
function getExeContext(prog)
{
    ...
    for (var i = 0; i < t16Chain.length; i++) {
        if (t16Chain[i] == prog.build.target.isa) {
            xdc.loadPackage(this.$module.T16.catalogName);
            cpu = ExeContext.create(this.$module.T16, this.$module.BOARD);
            core = "T16";
        }
    }
    if (cpu == undefined) {
...
/*
 *  @(#) ti.platforms.simArctic; 1, 0, 0,1; 12-3-2010 12:58:00; /db/ztree/library/trees/platform/platform-l29x/src/
 */
