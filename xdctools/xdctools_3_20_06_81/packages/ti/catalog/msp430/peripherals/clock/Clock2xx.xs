 ...
 *  ======== instance$meta$init ========
 */
function instance$meta$init(interruptEnableRegister1)
{
    this.interruptEnableRegister1 = interruptEnableRegister1;
    ...
    function calculateClock() {
        ...
        if (inst.XT2CLKHz > 0) {
            inst.hasXT2 = true;
            inst.BCSCTL1.XT2OFF = Clock2xx.XT2OFF_OFF;
        }
        else {
            inst.hasXT2 = false;
            inst.BCSCTL1.XT2OFF = Clock2xx.XT2OFF;
        }
                

        if (inst.BCSCTL1.XTS & Clock2xx.XTS) {   /* If XTS == 1 */
            $trace("if XTS == 1");
            ...
            switch (inst.BCSCTL3.LFXT1S) {
                case Clock2xx.LFXT1S_0:
                    inputHz = inst.LFXT1CLKHz;
                    break;
                case Clock2xx.LFXT1S_1:
                    inputHz = 0;
                    break;
                case Clock2xx.LFXT1S_2:
                    inputHz = inst.VLOCLKHz;
                    break;
                case Clock2xx.LFXT1S_3:
                    inputHz = inst.LFXT1CLKHz;
                    break;
            }    
        }
        ...
        switch (inst.BCSCTL1.DIVA) {
            case Clock2xx.DIVA_0:
                inst.ACLKHz = inputHz / 1;
                break;
            case Clock2xx.DIVA_1:
                inst.ACLKHz = inputHz / 2;
                break;
            case Clock2xx.DIVA_2:
                inst.ACLKHz = inputHz / 4;
                break;
            case Clock2xx.DIVA_3:
        ...
        switch (inst.BCSCTL2.SELM) {
            case Clock2xx.SELM_0:
            //case Clock2xx.SELM_1:
                inputHz = inst.DCOCLKHz;
                break;
            case Clock2xx.SELM_2:
                inputHz = inst.hasXT2 ? inst.XT2CLKHz : inst.LFXT1CLKHz;
                break;
            case Clock2xx.SELM_3:
                inputHz = inst.LFXT1CLKHz;
                break;
        }
        ...
        switch (inst.BCSCTL2.DIVM) {
            case Clock2xx.DIVM_0:
                inst.MCLKHz = inputHz / 1;
                break;
            case Clock2xx.DIVM_1:
                inst.MCLKHz = inputHz / 2;
                break;
            case Clock2xx.DIVM_2:
                inst.MCLKHz = inputHz / 4;
                break;
            case Clock2xx.DIVM_3:
        ...
        /********* Calculate SMCLK *********/
        if (inst.BCSCTL2.SELS & Clock2xx.SELS) {
            $trace("Inside SELS");
            inputHz = inst.hasXT2 ? inst.XT2CLKHz : inst.LFXT1CLKHz;    
        }
        else {
        ...
        switch (inst.BCSCTL2.DIVS) {
            case Clock2xx.DIVS_0:
                inst.SMCLKHz = inputHz / 1;
                break;
            case Clock2xx.DIVS_1:
                inst.SMCLKHz = inputHz / 2;
                break;
            case Clock2xx.DIVS_2:
                inst.SMCLKHz = inputHz / 4;
                break;
            case Clock2xx.DIVS_3:
                inst.SMCLKHz = inputHz / 8;
                break;
        }
        inst.$seal("SMCLKHz");

        $trace("inst.hasXT2 = " + inst.hasXT2);
    ...
    group.onSet(inst, "hasXT2", calculateClock);
    ...
    {
//        inst.preCalibratedValues = Clock2xx.PreCal_4;
        if(inst.preCalibratedValues == Clock2xx.PreCal_4)
        {
     ...
     */
    function updatePreCalibratedValues()
    {
//        $trace("Inside updatePreCalibratedValues()");
        print("Executing updatePreCalibratedValues()");
        protectDcoModRsel = 1;
        inst.$unseal("DCOCLKHz");
        if(inst.preCalibratedValues == Clock2xx.PreCal_0)
        {
            $trace("Inside PreCal_0");
            inst.DCOCLKHz = 1000000;
        }
        else if(inst.preCalibratedValues == Clock2xx.PreCal_1)
        {
            $trace("Inside PreCal_1");
            inst.DCOCLKHz = 8000000;
        }
        else if(inst.preCalibratedValues == Clock2xx.PreCal_2)
        {
            $trace("Inside PreCal_2");
            inst.DCOCLKHz = 12000000;
        }
        else if(inst.preCalibratedValues == Clock2xx.PreCal_3)
        {
            $trace("Inside PreCal_3");
            inst.DCOCLKHz = 16000000;
        }
        else if(inst.preCalibratedValues == Clock2xx.PreCal_4)
        {
...
function computeDCOCLKHz(targetValue)
{
    ...
    {
        preCalibratedValues = Clock2xx.PreCal_0;
        DCOCLKHz = targetValue;
    }
    else if(targetValue ==  8000000)
    {
        preCalibratedValues = Clock2xx.PreCal_1;
        DCOCLKHz = targetValue;
    }
    else if(targetValue == 12000000)
    {
        preCalibratedValues = Clock2xx.PreCal_2;
        DCOCLKHz = targetValue;
    }
    else if(targetValue == 16000000)
    {
        preCalibratedValues = Clock2xx.PreCal_3;
        DCOCLKHz = targetValue;
    }
    else
    {
        $trace("set this.preCalibratedValues = Clock2xx.PreCal_4");
        preCalibratedValues = Clock2xx.PreCal_4;
        DCOCLKHz = _configDCOCLKHz(this, targetValue);
    }

    this.preCalibratedValues = preCalibratedValues;
    print("Unseal DCOCLKHz");
...
{
    $trace("executing inside _configDCOCLKHz() with targetValue = " + targetValue);
    if((targetValue != 1000000)  || (targetValue != 8000000) ||
       (targetValue != 12000000) || (targetValue != 16000000))
    {
...
{
    $trace("executing function _calculateDCOFrequency; RSELValue = " + localRSELValue +
          " DCOBitValue = " + localDCOBitValue + " MODValue = " + localMODValue);
...
function _setDcoRselModBits(inst, flag)
{
    ...
    if(flag == 0)
    {
        ...
        */
        localDCOBitValue = 0;
        if (inst.DCOCTL.DCO0 & Clock2xx.DCO0) {
            localDCOBitValue = localDCOBitValue + 1;
        }
        if (inst.DCOCTL.DCO1 & Clock2xx.DCO1) {
            localDCOBitValue = localDCOBitValue + 2;
        }
        if (inst.DCOCTL.DCO2 & Clock2xx.DCO2) {
        ...
        */
        localRSELValue = 0;
        if (inst.BCSCTL1.RSEL0 & Clock2xx.RSEL0) {
            localRSELValue = localRSELValue + 1;
        }
        if (inst.BCSCTL1.RSEL1 & Clock2xx.RSEL1) {
            localRSELValue = localRSELValue + 2;
        }
        if (inst.BCSCTL1.RSEL2 & Clock2xx.RSEL2) {
            localRSELValue = localRSELValue + 4;
        }
        if (inst.BCSCTL1.RSEL3 & Clock2xx.RSEL3) {
        ...
        */   
        localMODValue = 0;
        if (inst.DCOCTL.MOD0 & Clock2xx.MOD0) {
            localMODValue = localMODValue + 1;
        }
        if (inst.DCOCTL.MOD1 & Clock2xx.MOD1) {
            localMODValue = localMODValue + 2;
        }
        if (inst.DCOCTL.MOD2 & Clock2xx.MOD2) {
            localMODValue = localMODValue + 4;
        }
        if (inst.DCOCTL.MOD3 & Clock2xx.MOD3) {
            localMODValue = localMODValue + 8;
        }
        if (inst.DCOCTL.MOD4 & Clock2xx.MOD4) {
    ...
    else if(flag == 1)
    {
        ...
        if(DCOBitValue & 0x4)
        {
            inst.DCOCTL.DCO2 = Clock2xx.DCO2;
        }
        else
        {
            inst.DCOCTL.DCO2 = Clock2xx.DCO2_OFF;
        }
        if(DCOBitValue & 0x2)
        {
            inst.DCOCTL.DCO1 = Clock2xx.DCO1;
        }
        else
        {
            inst.DCOCTL.DCO1 = Clock2xx.DCO1_OFF;
        }
        if(DCOBitValue & 0x1)
        {
            inst.DCOCTL.DCO0 = Clock2xx.DCO0;
        }
        else
        {
            inst.DCOCTL.DCO0 = Clock2xx.DCO0_OFF;
        }
        ...
        if(RSELValue & 0x8)
        {
            inst.BCSCTL1.RSEL3 = Clock2xx.RSEL3;
        }
        else
        {
            inst.BCSCTL1.RSEL3 = Clock2xx.RSEL3_OFF;
        }
        if(RSELValue & 0x4)
        {
            inst.BCSCTL1.RSEL2 = Clock2xx.RSEL2;
        }
        else
        {
            inst.BCSCTL1.RSEL2 = Clock2xx.RSEL2_OFF;
        }
        if(RSELValue & 0x2)
        {
            inst.BCSCTL1.RSEL1 = Clock2xx.RSEL1;
        }
        else
        {
            inst.BCSCTL1.RSEL1 = Clock2xx.RSEL1_OFF;
        }
        if(RSELValue & 0x1)
        {
            inst.BCSCTL1.RSEL0 = Clock2xx.RSEL0;
        }
        else
        {
            inst.BCSCTL1.RSEL0 = Clock2xx.RSEL0_OFF;
        }    
        ...
        if(MODValue & 0x10)
        {
            ...
            inst.DCOCTL.MOD4 = Clock2xx.MOD4;
        }
        else
        {
            $trace("modifying MOD4_OFF bit");
            inst.DCOCTL.MOD4 = Clock2xx.MOD4_OFF;
        }
        if(MODValue & 0x8)
        {
            $trace("modifying MOD3 bit");
            inst.DCOCTL.MOD3 = Clock2xx.MOD3;
        }
        else
        {
            $trace("modifying MOD3_OFF bit");
            inst.DCOCTL.MOD3 = Clock2xx.MOD3_OFF;
        }
        if(MODValue & 0x4)
        {
            $trace("modifying MOD2 bit");
            inst.DCOCTL.MOD2 = Clock2xx.MOD2;
        }
        else
        {
            $trace("modifying MOD2_OFF bit");
            inst.DCOCTL.MOD2 = Clock2xx.MOD2_OFF;
        }
        if(MODValue & 0x2)
        {
            $trace("modifying MOD1 bit");
            inst.DCOCTL.MOD1 = Clock2xx.MOD1;
        }
        else
        {
            $trace("modifying MOD1_OFF bit");
            inst.DCOCTL.MOD1 = Clock2xx.MOD1_OFF;
        }
        if(MODValue & 0x1)
        {
            $trace("modifying MOD0 bit");
            inst.DCOCTL.MOD0 = Clock2xx.MOD0;
        }
        else
        {
            $trace("modifying MOD0_OFF bit");
            inst.DCOCTL.MOD0 = Clock2xx.MOD0_OFF;
        }
    }    
