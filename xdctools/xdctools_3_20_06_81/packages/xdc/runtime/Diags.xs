...
/*
 *  ======== needsDiagsMask ========
 *  Retuns non-zero if module needs a runtime mask
 */
...
function module$use()
{
    ...
    if (rts) {
        var rtsPkg = xdc.loadPackage(rts);
        var xml = null;
...
function module$static$init(obj, params)
{
    ...
    for each (var mod in Program.targetModules()) {
        ...
        for each (var dc in CATEGORIES) {
            ...
            switch (mod.common$['diags_' + dc]) {
                ...
                case this.RUNTIME_OFF:
                    dincl |= this[dc];
                    if (!Program.frozenRomConfig2(this, 'setMaskEnabled')) {
        ...
        Program.freezeRomConfig2(mod, 'Module__diagsEnabled');
        Program.freezeRomConfig2(mod, 'Module__diagsIncluded');
        Program.freezeRomConfig2(mod, 'Module__diagsMask');
    ...
    if (this.setMaskEnabled) {
        ...
        if (Text.isLoaded) {
            ...
            /* setMask needs the strings on the target */
