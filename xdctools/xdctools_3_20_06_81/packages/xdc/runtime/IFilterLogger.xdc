 ...
 *  however, so it is not required that all ILogger implementations support it.
 *
 *  ILoggers which do intend to support filtering by detail level should 
 *  implement this interface, which extends the ILogger interface.
 *
 *  The {@link #setFilterLevel} and {@link #getFilterLevel} APIs support
 *  specification of a different filtering level for each of the diagnostics
 *  categories.
 */
