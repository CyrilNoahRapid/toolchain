...
function finalize()
{
	...
	if (mod instanceof xdc.om["xdc.runtime.ILogger"].Module) {
         ...
         */
        if (mod.$$scope != -1 && mod.$spec.needsRuntime()) {
        ...
        Program.freezeRomConfig2(mod, 'Object__sizeof');
        if (mod.common$.instanceHeap == null) {
            Program.freezeRomConfig2(mod, 'Object__heap');
        }
        if (mod.common$.logger == null) {
            Program.freezeRomConfig2(mod, 'Module__loggerObj');
        }
        if (mod.Module__gateObj == null) {
            Program.freezeRomConfig2(mod, 'Module__gateObj');
        }
        if (mod.Module__gatePrms == null) {
            Program.freezeRomConfig2(mod, 'Module__gatePrms');
        }

        if (!mod.$$instflag) {
            Program.freezeRomConfig2(mod, 'Object__count');
            Program.freezeRomConfig2(mod, 'Object__table');
        }
    }
}
...
/*
 *  @(#) xdc.runtime; 2, 1, 0,296; 11-16-2010 11:20:25; /db/ztree/library/trees/xdc/xdc-v53x/src/packages/
 */
