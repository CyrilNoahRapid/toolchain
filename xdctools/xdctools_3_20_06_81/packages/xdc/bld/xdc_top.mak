...
# If we don't replace XDCBUILDCFG with the new value, sub-make will get the
# value passsed on the command line.
MAKEOVERRIDES := $(patsubst XDCBUILDCFG=%,XDCBUILDCFG=$(XDCBUILDCFG),$(MAKEOVERRIDES))
...
$(2),$(1):
	@$(if $(QFLAG),,echo ======== $(1) [$(2)] ========)
	+$(if $(KFLAG),-)@"$(MAKE)" -r -R -C $(2) --no-print-directory -f "$(XDCROOT)/packages/xdc/bld/xdc.mak" XDCROOT="$(XDCROOT)" HOSTOS=$(HOSTOS) $(1)
