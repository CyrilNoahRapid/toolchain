...
function $$cfgUseModule(qname, cycles)
{
     ...
     * changed.
     * Also, if we didn't change the value of $curpkg to "undefined" when
     * getting the reference to xdc.cfg.Main, we would create an invalid
     * dependency between the package that called xdc.useModule(qname) and
     * 'xdc.cfg'. (This last point was valid until we switched to using
     * xdc.om[] access, but it may not be valid now.)
     *
     ...
     * package must be 'qname'. We could save $curpkg inside $$usemod but it
     * might be better to keep this part of $curpkg handling here to avoid
     * polluting too many functions with $curpkg handling.
     *
...
/*
 *  @(#) xdc.cfg; 1, 0, 2, 0,296; 11-16-2010 11:20:05; /db/ztree/library/trees/xdc/xdc-v53x/src/packages/
 */
