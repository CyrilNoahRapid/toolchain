...
function canUseProxy() {
    ...
    return "newInstance" in ModelDataProxy &&
        "OK" in SyncExecCommand;
}
...
function loadCfgModel() 
{
        ...
        for (var x in xdcGlobal) {
            ...
            if ((gid === undefined) || (typeof(gid) != 'object') || !('$category' in gid)) {
                continue;
            }
            ...
            /* add an id for it */
            var nid = new java.lang.String(x);
        ...
        this.$private.rootNode = new ConfigNode(this.$private.exeName,
                this.$private.exeName, null, null, null, false,
...
function addConfigs(obj, node)
{
 ...
 //           java.lang.System.out.println('node.instance.name: ' + obj.instance + ' = ' + tv.name);
            ...
            var isn = Type.findModSpec('xdc.runtime.IInstance');
            ...
            if (cfg != null) {
                for (var i = 0; i < cfg.size(); i++) {
                    ...
                    var tsn = Type.findModPropSpec('xdc.runtime.IInstance', tnm);
...
function addCreateArgs( obj, node )
{
    ...
    if (('$args' in obj) && (argSpecs != null)) {
        ...
        for (var i = 0; i < argSpecs.size(); i++) {
            ...
            var argNode = new ConfigNode(n, n, null, md, argType, false,
                    node.getConfigModel());
